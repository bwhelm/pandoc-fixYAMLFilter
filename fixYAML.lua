...
function fixYAMLHeader(meta)
    if FORMAT ~= 'latex' or FORMAT ~= 'beamer' then  -- Need to cope with csl style....
        if meta.csl then  -- Use the one that's provided if it is
            meta.csl[1].text = string.gsub(meta.csl[1].text, "~", os.getenv('HOME'))
        elseif not meta.csl then  -- otherwise choose inline or notes style.
            ...
            if meta.biblatexoptions and meta.biblatexoptions[1].text == 'authordate' then
    ...
    if meta.geometry and meta.geometry[1].text == 'ipad' then
        if meta.book then
            -- meta.geometry[1].text = 'paperwidth=176mm,paperheight=234mm,outer=22mm,top=2.5pc,bottom=3pc,headsep=1pc,includehead,includefoot,centering,inner=22mm,marginparwidth=17mm'
            meta.geometry[1].text = 'paperwidth=190.5mm,paperheight=271.88mm,outer=22mm,top=2.5pc,bottom=3pc,headsep=1pc,includehead,includefoot,centering,inner=22mm,marginparwidth=17mm'
        else
            -- meta.geometry[1].text = 'paperwidth=176mm,paperheight=234mm,width=360.0pt,height=541.40024pt,headsep=1pc,centering'
            ...
            meta.geometry[1].text = 'paperwidth=190.5mm,paperheight=271.88mm,width=136.95mm,height=221.09mm,headsep=1pc,centering'
        end
    end
    if meta.bibliography then  -- Get complete path to bibliography files
        if meta.bibliography.t == 'MetaList' then
            for key, value in pairs(meta.bibliography) do
                meta.bibliography[key][1].text = getPathToBibFile(value[1].text)
            end
        elseif meta.bibliography.t == 'MetaInlines' then
            meta.bibliography[1].text = getPathToBibFile(meta.bibliography[1].text)
        end
    end
    return meta
end
